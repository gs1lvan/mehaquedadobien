...
class ShoppingListManager {
    ...
    createList(name) {
        const list = {
            id: Date.now(),
            name: name,
            recipes: [],  // Array of recipe references with modifications
            manualItems: [],  // Manual items added by user
            items: [],  // Legacy support - will be deprecated
            enabled: true,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        };
    ...
    /**
     * Add recipe reference to shopping list
     * @param {number} listId - List ID
     * @param {number} recipeId - Recipe ID
     * @param {string} day - Day name
     * @param {number} menuId - Menu ID (optional, for menu-sourced recipes)
     * @param {string} source - Source type: 'menu' or 'recipe' (default: 'recipe')
     * @returns {Object|null} The added recipe reference
     */
    addRecipeReference(listId, recipeId, day = '', menuId = null, source = 'recipe') {
        const list = this.getList(listId);
        if (!list) {
            console.error('[ShoppingListManager] List not found:', listId);
            return null;
        ...
        const recipeRef = {
            ...
            recipeId: recipeId,
            day: day,
            menuId: menuId,  // NEW: Track which menu this recipe comes from
            source: source,  // NEW: Track source type ('menu' or 'recipe')
            modifications: {}  // { ingredientId: { completed, customQuantity, deleted } }
        };

        if (!list.recipes) list.recipes = [];
        list.recipes.push(recipeRef);
        list.updatedAt = new Date().toISOString();
        this.saveLists();

        console.log('[ShoppingListManager] Added recipe reference:', recipeId, day, 'from', source, menuId ? `(menu: ${menuId})` : '');
        return recipeRef;
    }

    /**
     * Remove recipe reference from shopping list
     * @param {number} listId - List ID
     * @param {number} recipeRefId - Recipe reference ID
     * @returns {boolean} Success status
     */
    removeRecipeReference(listId, recipeRefId) {
        const list = this.getList(listId);
        if (!list || !list.recipes) {
            console.error('[ShoppingListManager] List or recipes not found:', listId);
            return false;
        }

        const index = list.recipes.findIndex(r => r.id === recipeRefId);
        if (index === -1) {
            console.error('[ShoppingListManager] Recipe reference not found:', recipeRefId);
            return false;
        }
        ...
        console.log('[ShoppingListManager] Removed recipe reference:', recipeRefId);
        return true;
    }

    /**
     * Update ingredient modification in recipe
     * @param {number} listId - List ID
     * @param {number} recipeRefId - Recipe reference ID
     * @param {string} ingredientId - Ingredient ID
     * @param {Object} modification - Modification object { completed, customQuantity, deleted }
     * @returns {boolean} Success status
     */
    updateIngredientModification(listId, recipeRefId, ingredientId, modification) {
        const list = this.getList(listId);
        if (!list || !list.recipes) {
            console.error('[ShoppingListManager] List or recipes not found:', listId);
            return false;
        ...
        const recipeRef = list.recipes.find(r => r.id === recipeRefId);
        if (!recipeRef) {
            console.error('[ShoppingListManager] Recipe reference not found:', recipeRefId);
            return false;
        }

        recipeRef.modifications[ingredientId] = {
            ...recipeRef.modifications[ingredientId],
            ...modification
        };
        ...
        console.log('[ShoppingListManager] Updated ingredient modification:', ingredientId, modification);
        return true;
    }
    ...
    getTotalCount(listId) {
        ...
        // Count legacy items
        let count = list.items ? list.items.length : 0;

        // Count manual items
        if (list.manualItems) {
            count += list.manualItems.length;
        }

        // Count recipe ingredients (not deleted)
        if (list.recipes) {
            list.recipes.forEach(recipeRef => {
                // This will be calculated dynamically when we have access to recipes
                // For now, just count as 1 per recipe
                count += 1;
            });
        }
...
class RecipeApp {
    ...
    addRecipeToShoppingList(recipeId, shoppingListId) {
        ...
        // Add recipe reference (new system)
        this.shoppingListManager.addRecipeReference(shoppingListId, recipeId, '');

        this.showSuccess(`Receta "${recipe.name}" añadida a la lista`);
        console.log(`[Shopping] Added recipe reference:`, recipe.name);

        // Re-render to show the new recipe
    ...
    /**
     * Convert menu to shopping list - shows modal to select list
     * @param {number} menuId - Menu ID
     */
    convertMenuToShoppingList(menuId) {
        console.log('🛒🛒🛒 [Menu Conversion] STARTING - MenuID:', menuId);
        ...
        console.log('🛒🛒🛒 [Menu Conversion] Menu found:', menu.name, '| Items:', menu.items.length);

        // Store menu ID for later use
        this.pendingMenuForShoppingList = menuId;

        // Show modal to select shopping list
        this.showSelectShoppingListModalForMenu(menu);
    }

    /**
     * Add menu recipes to shopping list
     * @param {number} listId - Shopping list ID
     * @param {number} menuId - Menu ID
     */
    addMenuRecipesToShoppingList(listId, menuId) {
        const menu = this.menuManager.getMenu(menuId);
        if (!menu) {
            console.error('[Add Menu Recipes] Menu not found:', menuId);
            this.showError('Menú no encontrado');
            return;
        }

        // Process each menu item (day) and add recipe references
        let recipesAdded = 0;
        let recipesNotFound = 0;

        console.log('🛒 [Add Menu Recipes] Processing menu items:', menu.items.length);

        menu.items.forEach((item, index) => {
            const dayName = item.name || 'Sin día';
            console.log(`🛒 [Add Menu Recipes] Processing day ${index + 1}:`, dayName);
            ...
            if (lunchId) {
                ...
                if (recipe && recipe.ingredients && recipe.ingredients.length > 0) {
                    this.shoppingListManager.addRecipeReference(listId, lunchId, dayName, menuId, 'menu');
                    recipesAdded++;
                    console.log(`🛒 [Add Menu Recipes] ✅ Added lunch recipe: ${recipe.name} for ${dayName}`);
                } else if (lunchId) {
                    console.warn(`[Add Menu Recipes] ❌ Recipe not found or has no ingredients: ${lunchId} for ${dayName}`);
                    recipesNotFound++;
                }
            }
            ...
            if (dinnerId) {
                ...
                if (recipe && recipe.ingredients && recipe.ingredients.length > 0) {
                    this.shoppingListManager.addRecipeReference(listId, dinnerId, dayName, menuId, 'menu');
                    recipesAdded++;
                    console.log(`🛒 [Add Menu Recipes] ✅ Added dinner recipe: ${recipe.name} for ${dayName}`);
                } else if (dinnerId) {
                    console.warn(`[Add Menu Recipes] ❌ Recipe not found or has no ingredients: ${dinnerId} for ${dayName}`);
                    recipesNotFound++;
                }
            }
        });
        ...
        if (recipesAdded === 0) {
            this.showError('Este menú no tiene recetas con ingredientes');
            this.closeSelectShoppingListModal();
            return;
        }

        console.log(`[Add Menu Recipes] Added ${recipesAdded} recipes from menu "${menu.name}"`);
        ...
        if (recipesNotFound > 0) {
            this.showToast(`${recipesAdded} recetas añadidas (${recipesNotFound} recetas no encontradas)`, 'warning');
        } else {
            this.showSuccess(`${recipesAdded} recetas añadidas del menú`);
        }

        // Switch to shopping lists view
        this.showShoppingListsView();

        // Re-render to show the new recipes
        this.renderShoppingLists();
    ...
    renderCategorySelectorChips(preSelectCategory = true, isQuickEdit = false, currentRecipeName = '') {
        ...
        // Get all categories (excluding special ones like caravana and hospital)
        const categories = this.categoryManager.getAllCategories(true)
            .filter(cat => !cat.isSpecial);

        console.log('🎨 [Category Selector] Rendering', categories.length, 'categories');

        categories.forEach(category => {
            // Check if category has recipes
            const recipesInCategory = this.recipes.filter(recipe => recipe.category === category.id);
            const hasAnyRecipes = recipesInCategory.length > 0;
            ...
            if (!hasRecipes) {
                ...
                chip.style.cursor = 'not-allowed';
                chip.disabled = true;
                console.log('🎨 [Category Selector] Disabled category:', category.name, '(no recipes)');
            }
    ...
    */
    ...
    renderRecipesGroupedByDay(recipes, menu, container) {
        ...
        menu.items.forEach((item, index) => {
            ...
            const dayNumber = index + 1;  // Use index + 1 to match metadata
            // Use item.id as key for unique identification
            const dayKey = item.id;
        ...
        console.log('[Menu Filter] === MATCHING RECIPES ===');
        ...
        recipes.forEach(recipe => {
            ...
            if (recipeMeta && recipeMeta.length > 0) {
                ...
                recipeMeta.forEach(meta => {
                    // Use item.id as key instead of dayNumber_day
                    const dayKey = meta.itemId;
                    console.log('[Menu Filter]   Trying dayKey (itemId):', dayKey);
                    const dayData = recipesByDay.get(dayKey);

                    if (dayData) {
                        console.log('[Menu Filter]   ✓ Found dayData for:', meta.day);
                        if (meta.mealType === 'lunch') {
                            dayData.lunch = recipe;
                            console.log('[Menu Filter]   ✓ Assigned to lunch');
                        } else if (meta.mealType === 'dinner') {
                            dayData.dinner = recipe;
                            console.log('[Menu Filter]   ✓ Assigned to dinner');
                        }
                    } else {
                        console.warn('[Menu Filter]   ✗ dayData NOT found for dayKey:', dayKey);
                        console.warn('[Menu Filter]   Available dayKeys:', Array.from(recipesByDay.keys()));
                    }
                });
            } else {
        ...
        sortedDays.forEach(dayData => {
            ...
            const getMealCard = (mealValue, recipeObj) => {
                ...
                if (recipeObj) {
                    console.log('[Menu Filter] Creating card for recipe:', recipeObj.name, 'Images:', recipeObj.images?.length || 0);
                ...
                // Parse the category value (e.g., "🐷 Cerdo")
                const parts = mealValue.split(' ');
                const categoryEmoji = parts[0] || '🍽️';
                const categoryName = parts.slice(1).join(' ') || 'Categoría';
                ...
                placeholderCard.appendChild(categorySpan);
    ...
    renderDetailAdditionalInfo(author, history) {
        ...
        if (hasAuthor) {
            authorSection.classList.remove('u-hidden');
            authorElement.textContent = author;
        } else {
            authorSection.classList.add('u-hidden');
        }

        // Show history if available
        if (hasHistory) {
            historySection.classList.remove('u-hidden');
            historyElement.textContent = history;
        } else {
            historySection.classList.add('u-hidden');
        }
    }
    ...
    renderDetailMultimedia(images, videos, recipeName = '', totalTime = '', category = '', caravanFriendly = false, hospitalFriendly = false, menuFriendly = false) {
        ...
        if (!sectionElement || !imagesGallery) return;
    ...
    setupDetailEventListeners(recipe) {
        ...
        if (addAllIngredientsBtn) {
            addAllIngredientsBtn.onclick = () => {
                this.showSelectShoppingListModalForAllIngredients(recipe);
            };
        }
    }
        ...
        if (addItemBtn) {
            addItemBtn.addEventListener('click', () => {
                ...
                if (existingContainer) {
                    totalItems += existingContainer.querySelectorAll('.menu-item-input').length;
                }
                if (newContainer) {
                    totalItems += newContainer.querySelectorAll('.menu-item-input').length;
                }

                console.log('🟡 [Add Menu Item] Total items:', totalItems);

                if (totalItems >= 7) {
                    this.showToast('No puedes añadir más de 7 días a un menú', 'warning');
                    return;
                }
    ...
    renderMenuCard(menu, isHidden = false) {
        console.log('[renderMenuCard] Rendering menu:', menu.id, menu.name);
        const card = document.createElement('div');
        card.className = 'shopping-list-card';
        card.dataset.menuId = menu.id;
        console.log('[renderMenuCard] Card dataset.menuId set to:', card.dataset.menuId);
        ...
        bookmarkBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            e.preventDefault();
            console.log('[Bookmark] Button clicked for menu:', menu.id);
        ...
        header.addEventListener('click', (e) => {
            console.log('[Header Click] Clicked on menu:', menu.id, menu.name);
            ...
            console.log('[Header Click] Calling toggleMenuExpanded for:', menu.id);
    ...
    renderMenuItems(menu) {
        console.log('[renderMenuItems] Rendering items for menu:', menu.id, menu.name);
        const container = document.createElement('div');
    ...
    showMenuOptionsModal(menuId) {
        ...
        const toShoppingListBtn = document.getElementById('menu-option-to-shopping-list');
        ...
        if (toShoppingListBtn) {
            toShoppingListBtn.onclick = () => {
                this.closeMenuOptionsModal();
                const newList = this.convertMenuToShoppingList(menuId);

                if (newList) {
                    // Switch to shopping lists view
                    this.showShoppingListsView();

                    // Re-render shopping lists to show the new one
                    this.renderShoppingLists();

                    // Expand the newly created list
                    setTimeout(() => {
                        this.expandList(newList.id);
                    }, 100);
                }
            };
        }
                ...
                if (this.selectedRecipeId && this.currentMenuRecipeInput) {
                    ...
                    if (selectedRecipe) {
                        ...
                        // Check if this is quick edit mode
                            ...
                            console.log('🔵 [Quick Edit] Menu object:', menu);
                            ...
                            if (menu) {
                                ...
                                console.log('🔵 [Quick Edit] Item before update:', JSON.stringify(item));
                                ...
                                if (item) {
                                    ...
                                    console.log('🔵 [Quick Edit] After setRecipeForMeal:', mealType, '=', mealType === 'lunch' ? item.lunch : item.dinner);
                                    console.log('🔵 [Quick Edit] Recipe ID set:', mealType === 'lunch' ? item.lunchId : item.dinnerId);
                                    console.log('🔵 [Quick Edit] Item after update:', JSON.stringify(item));
                                    ...
                                    console.log('🔵 [Quick Edit] Updated input field value to:', selectedRecipe.name);
                                    ...
                                    console.log('🔵 [Quick Edit] Current view check:');
                                    console.log('  - recipesView hidden?', recipesView?.classList.contains('hidden'));
                                    console.log('  - menusView hidden?', menusView?.classList.contains('hidden'));
                                    console.log('  - menuModal hidden?', menuModal?.classList.contains('hidden'));
                                    ...
                                    if (recipesView && !recipesView.classList.contains('hidden')) {
                                        ...
                                        // We're in the menu edit modal - need to refresh it
                                        console.log('🔵 [Quick Edit] Menu modal is open, refreshing it');
                                        // The input field is already updated above, no need to re-render the whole modal
                                    }

                                    this.showToast('Receta actualizada correctamente', 'success');
                                } else {
                                    console.error('🔴 [Quick Edit] Item not found in menu!');
                                    console.error('🔴 [Quick Edit] Looking for itemId:', itemId, 'type:', typeof itemId);
                                    console.error('🔴 [Quick Edit] Available items:', menu.items.map(i => ({ id: i.id, type: typeof i.id })));
                                }
                            } else {
        ...
        if (menuId) {
            ...
            if (menu) {
                ...
                if (menu.items && menu.items.length > 0) {
                    // Sort items by day of week before rendering
                    const sortedItems = this.sortMenuItemsByDay(menu.items);
                    console.log('📅 [Menu Form] Sorted items by day:', sortedItems.map(i => i.name));
    ...
    /**
     * Sort menu items by day of week
     * @param {Array} items - Menu items to sort
     * @returns {Array} Sorted items
     */
    sortMenuItemsByDay(items) {
        const daysOfWeek = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'];
        ...
        return items.slice().sort((a, b) => {
            ...
            // If day not found in array, put it at the end
    ...
    addMenuItemInput(item = null, isExisting = false) {
        ...
        lunchInput.addEventListener('click', () => {
            console.log('🍽️ [Menu Item] Lunch input clicked, isExisting:', isExisting, 'item:', item);
        ...
        dinnerInput.addEventListener('click', () => {
            console.log('🍽️ [Menu Item] Dinner input clicked, isExisting:', isExisting, 'item:', item);
    ...
    saveMenu() {
        ...
        // Sort items by day of week before saving
        const sortedItems = this.sortMenuItemsByDay(items);
        console.log('📅 [Save Menu] Sorted items by day:', sortedItems.map(i => i.name));
        ...
        if (this.currentMenuId) {
            ...
            // Create new menu
            const newMenu = this.menuManager.createMenu(menuName, sortedItems);
    ...
    renderShoppingItems(list) {
        ...
        const hasRecipes = list.recipes && list.recipes.length > 0;
        const hasManualItems = list.manualItems && list.manualItems.length > 0;
        const hasLegacyItems = list.items && list.items.length > 0;

        // If list has new format (recipes or manualItems), ignore legacy items
        const useNewFormat = hasRecipes || hasManualItems;

        if (!hasRecipes && !hasManualItems && !hasLegacyItems) {
            const empty = document.createElement('p');
            empty.style.color = 'var(--color-text-secondary)';
            empty.style.fontSize = '0.875rem';
        ...
        console.log('🎨 [Render] Rendering list:', list.name, '| Recipes:', list.recipes?.length || 0, '| Manual:', list.manualItems?.length || 0, '| UseNewFormat:', useNewFormat);

        // Render recipe references grouped by source
        if (hasRecipes) {
            // Separate recipes by source
            const menuRecipes = list.recipes.filter(r => r.source === 'menu');
            const recipeBookRecipes = list.recipes.filter(r => r.source === 'recipe' || !r.source);

            // Render MENÚS section
            if (menuRecipes.length > 0) {
                this.renderSectionSeparator(container, '📋', 'MENÚS');
                this.renderRecipesByDay(container, menuRecipes, list.id);
            }

            // Render RECETARIO section
            if (recipeBookRecipes.length > 0) {
                this.renderSectionSeparator(container, '📖', 'RECETARIO');
                this.renderRecipesByDay(container, recipeBookRecipes, list.id);
            }
        }

        // Render OTRAS COMPRAS section
        if (hasManualItems) {
            this.renderSectionSeparator(container, '🛒', 'OTRAS COMPRAS');

            const manualSection = document.createElement('div');
            manualSection.className = 'shopping-recipe-group';
            ...
            list.manualItems.forEach(item => {
                const itemDiv = this.createManualShoppingItem(list.id, item);
                manualSection.appendChild(itemDiv);
            });

            container.appendChild(manualSection);
        }

        // Render legacy items ONLY if not using new format
        if (hasLegacyItems && !useNewFormat) {
            console.log('🎨 [Render] Rendering legacy items:', list.items.length);
            let currentRecipeGroup = null;

            list.items.forEach((item, index) => {
                console.log(`🎨 [Render] Item ${index}:`, item.name);

                // Check for recipe markers
                if (item.name.startsWith('[RECIPE_START]')) {
                    console.log('🎨 [Render] ✅ Found RECIPE_START marker');
                    ...
                    currentRecipeGroup.style.cssText = `
                    background: var(--color-background-secondary);
                    border: 1px solid var(--color-border);
                    border-radius: 12px;
                    padding: 1rem;
                ...
                if (isRecipeItem) {
                    itemDiv.classList.add('shopping-recipe-item');
                }
                ...
                itemName.textContent = isRecipeItem ? item.name.replace('[RECIPE_ITEM]', '') : item.name;
                ...
                // Add delete button
                const deleteBtn = document.createElement('button');
                deleteBtn.className = 'btn-icon shopping-item-delete';
                deleteBtn.innerHTML = '<i class="fa-solid fa-trash"></i>';
                deleteBtn.title = 'Eliminar';
                deleteBtn.onclick = (e) => {
                    e.stopPropagation();
                    this.deleteShoppingItem(list.id, index);
                };
                itemDiv.appendChild(deleteBtn);

                // Add to recipe group or main container
                if (currentRecipeGroup && isRecipeItem) {
                    currentRecipeGroup.appendChild(itemDiv);
                } else {
                    container.appendChild(itemDiv);
                }
            });
     ...
     */
    renderSectionSeparator(container, icon, title) {
        const separator = document.createElement('div');
        separator.className = 'shopping-section-separator';
        separator.style.cssText = `
            display: flex;
            align-items: center;
            gap: 1rem;
        ...
        const lineBefore = document.createElement('div');
        lineBefore.style.cssText = 'flex: 0 0 auto; width: 40px; height: 3px; background: var(--color-text);';

        const lineAfter = document.createElement('div');
        lineAfter.style.cssText = 'flex: 1; height: 3px; background: var(--color-text);';

        separator.appendChild(lineBefore);
        separator.appendChild(iconSpan);
    ...
    /**
     * Render recipes grouped by day
     * @param {HTMLElement} container - Container element
     * @param {Array} recipes - Array of recipe references
     * @param {number} listId - List ID
     */
    renderRecipesByDay(container, recipes, listId) {
        // Group recipes by day
        const recipesByDay = new Map();

        recipes.forEach(recipeRef => {
        ...
        // Count ingredient occurrences across ALL recipes in this group
        ...
        recipesByDay.forEach((dayRecipes, day) => {
            ...
            dayRecipes.forEach(recipeRef => {
                ...
                deleteRecipeBtn.onclick = (e) => {
                    e.stopPropagation();
                    this.deleteRecipeFromShoppingList(listId, recipeRef.id);
                };
                ...
                if (recipe.ingredients && recipe.ingredients.length > 0) {
                    recipe.ingredients.forEach(ingredient => {
                        const mod = recipeRef.modifications[ingredient.id] || {};
                        if (ingredient.name.toLowerCase() === 'caldo') {
                            console.log('[Render DEBUG] Caldo found!');
                            console.log('[Render DEBUG] recipeRef.id:', recipeRef.id);
                            console.log('[Render DEBUG] ingredient.id:', ingredient.id);
                            console.log('[Render DEBUG] recipeRef.modifications:', recipeRef.modifications);
                            console.log('[Render DEBUG] mod:', mod);
                        }
                        ...
                        const key = ingredient.name.toLowerCase().trim();
                        const count = ingredientCounts.get(key) || 1;
    ...
    /**
     * Create shopping ingredient item element
     * @param {number} listId - List ID
     * @param {number} recipeRefId - Recipe reference ID
     * @param {Object} ingredient - Ingredient object
     * @param {Object} mod - Modifications object
     * @param {number} count - Number of times this ingredient appears (default: 1)
     * @returns {HTMLElement} Item element
     */
    createShoppingIngredientItem(listId, recipeRefId, ingredient, mod, count = 1) {
        const itemDiv = document.createElement('div');
        itemDiv.className = 'shopping-item';
        if (mod.completed) itemDiv.classList.add('completed');
        itemDiv.dataset.ingredientId = ingredient.id;

        // Checkbox for completed
        const checkbox = document.createElement('input');
        checkbox.type = 'checkbox';
        checkbox.checked = mod.completed || false;
        checkbox.onchange = () => {
            this.toggleIngredientCompleted(listId, recipeRefId, ingredient.id);
        };
        ...
        const itemName = document.createElement('span');
        itemName.className = 'shopping-item-name';
        itemName.textContent = ingredient.name;

        // Add count badge if ingredient appears more than once
        if (count > 1) {
            const countBadge = document.createElement('span');
            countBadge.className = 'ingredient-count-badge';
            countBadge.textContent = ` x${count}`;
            itemName.appendChild(countBadge);
        }

        itemContent.appendChild(itemName);

        // Quantity (editable badge - exactly same style as count badge)
        const quantity = mod.customQuantity || (ingredient.quantity + (ingredient.unit ? ' ' + ingredient.unit : ''));
        const itemQuantity = document.createElement('span');
        ...
        if (count > 1) {
            ...
            itemQuantity.onclick = () => {
                this.showIngredientMergeModal(listId, ingredient.name, count);
            };
        } else {
            itemQuantity.title = 'Click para editar cantidad';
            itemQuantity.onclick = () => {
        ...
        deleteBtn.className = 'btn-icon shopping-item-delete';
        deleteBtn.innerHTML = '<i class="fa-solid fa-trash"></i>';
        deleteBtn.title = 'Eliminar ingrediente';
        deleteBtn.onclick = (e) => {
            e.stopPropagation();
            this.deleteIngredientFromShoppingList(listId, recipeRefId, ingredient.id);
        };
        itemDiv.appendChild(deleteBtn);
    ...
    /**
     * Create manual shopping item element
     * @param {number} listId - List ID
     * @param {Object} item - Manual item object
     * @returns {HTMLElement} Item element
     */
    createManualShoppingItem(listId, item) {
        const itemDiv = document.createElement('div');
        itemDiv.className = 'shopping-item';
        ...
        checkbox.onchange = () => {
            this.toggleManualItemCompleted(listId, item.id);
        };
        ...
        deleteBtn.onclick = (e) => {
            e.stopPropagation();
            this.deleteManualItemFromShoppingList(listId, item.id);
        };
        itemDiv.appendChild(deleteBtn);
    ...
    /**
     * Toggle ingredient completed status
     */
    toggleIngredientCompleted(listId, recipeRefId, ingredientId) {
        ...
        const mod = recipeRef.modifications[ingredientId] || {};
        this.shoppingListManager.updateIngredientModification(
            listId,
            recipeRefId,
            ingredientId,
     ...
     */
    editIngredientQuantity(listId, recipeRefId, ingredientId, currentQuantity) {
        const newQuantity = prompt('Nueva cantidad:', currentQuantity);
        if (newQuantity === null) return;

        // Get the ingredient to find all related recipes
        const list = this.shoppingListManager.getList(listId);
        if (list) {
            const recipeRef = list.recipes.find(r => r.id === recipeRefId);
            if (recipeRef) {
                ...
                if (recipe) {
                    ...
                    if (ingredient) {
                        // Unmerge all instances of this ingredient
                        const allRecipesWithIngredient = this.findRecipesWithIngredient(listId, ingredient.name);
                        allRecipesWithIngredient.forEach(({ recipeRef: rRef, ingredient: ing }) => {
                            this.shoppingListManager.updateIngredientModification(
                                listId,
                                rRef.id,
                                ing.id,
        ...
        // Update modification with new quantity
        this.shoppingListManager.updateIngredientModification(
            listId,
            recipeRefId,
            ingredientId,
            { customQuantity: newQuantity, merged: false }
        );

        // Re-render and restore expanded state
        this.renderShoppingLists();

        // Force expand the list immediately
        setTimeout(() => {
            this.expandList(listId);
        }, 50);
    }
     ...
     */
    deleteIngredientFromShoppingList(listId, recipeRefId, ingredientId) {
        if (!confirm('¿Eliminar este ingrediente de la lista?')) return;

        // Save expanded state
        const wasExpanded = this.isListExpanded(listId);

        // Get ingredient name before deletion to recalculate merges
        const list = this.shoppingListManager.getList(listId);
        ...
        // Recalculate merged quantities for remaining instances
        if (ingredientName) {
            const remainingRecipes = this.findRecipesWithIngredient(listId, ingredientName);
            if (remainingRecipes.length > 0) {
                const mergedQuantity = this.calculateMergedQuantity(remainingRecipes);
     ...
     */
    deleteRecipeFromShoppingList(listId, recipeRefId) {
        if (!confirm('¿Eliminar esta receta completa de la lista?')) return;

        // Save expanded state
        const wasExpanded = this.isListExpanded(listId);

        // Get all ingredients from this recipe before deletion to recalculate merges
        const list = this.shoppingListManager.getList(listId);
            ...
            if (recipeRef) {
                ...
                if (recipe && recipe.ingredients) {
                    recipe.ingredients.forEach(ing => {
                        const mod = recipeRef.modifications[ing.id] || {};
                        if (!mod.deleted) {
        ...
        // Remove recipe
        this.shoppingListManager.removeRecipeReference(listId, recipeRefId);

        // Recalculate merged quantities for all affected ingredients
    ...
    /**
     * Find all recipes in list that contain a specific ingredient
     * @param {number} listId - Shopping list ID
     * @param {string} ingredientName - Ingredient name (normalized)
     * @returns {Array} Array of {recipeRef, recipe, ingredient, mod}
     */
    findRecipesWithIngredient(listId, ingredientName) {
        const list = this.shoppingListManager.getList(listId);
        ...
        list.recipes.forEach(recipeRef => {
            ...
            recipe.ingredients.forEach(ingredient => {
                ...
                if (ingName === normalizedName) {
                    const mod = recipeRef.modifications[ingredient.id] || {};
                    if (!mod.deleted) {
                        console.log('[Merge] Adding to results:', {
                            recipeName: recipe.name,
                            recipeRefId: recipeRef.id,
        ...
        console.log('[Merge] Found', results.length, 'recipes with ingredient:', ingredientName);
        return results;
    }

    /**
     * Show modal to merge ingredient quantities from multiple recipes
     * @param {number} listId - Shopping list ID
     * @param {string} ingredientName - Ingredient name
     * @param {number} count - Number of times ingredient appears
     */
    showIngredientMergeModal(listId, ingredientName, count) {
        console.log('[Merge Modal] Opening for:', ingredientName, 'count:', count);

        // Find all recipes with this ingredient
        const recipesWithIngredient = this.findRecipesWithIngredient(listId, ingredientName);

        if (recipesWithIngredient.length === 0) {
            this.showToast('No se encontraron recetas con este ingrediente', 'error');
            return;
        }

        // Store context
        this.currentMergeContext = {
            listId,
            ingredientName,
        ...
        this.renderMergeModalContent();

        // Setup event listeners
        this.setupMergeModalListeners();

        // Show modal
        const modal = document.getElementById('ingredient-merge-modal');
        if (modal) {
            modal.classList.remove('hidden');
        }
    }
    ...
    renderMergeModalContent() {
        const { ingredientName, recipes } = this.currentMergeContext;

        // Update title
        const title = document.getElementById('merge-modal-title');
        if (title) {
            title.textContent = `Fusionar: ${ingredientName}`;
        }

        // Render recipes list
        const container = document.getElementById('merge-recipes-list');
        if (!container) return;

        container.innerHTML = '';

        recipes.forEach(({ recipeRef, recipe, ingredient, mod }) => {
            const itemDiv = document.createElement('div');
            itemDiv.className = 'merge-recipe-item';
            itemDiv.dataset.recipeRefId = recipeRef.id;
            itemDiv.dataset.ingredientId = ingredient.id;

            // Create label with checkbox
            const label = document.createElement('label');
            label.className = 'merge-recipe-label';

            const checkbox = document.createElement('input');
            checkbox.type = 'checkbox';
            checkbox.className = 'merge-recipe-checkbox';
            checkbox.checked = mod.merged || false;
            checkbox.dataset.recipeRefId = recipeRef.id;
            checkbox.dataset.ingredientId = ingredient.id;

            const info = document.createElement('div');
            info.className = 'merge-recipe-info';

            const recipeName = document.createElement('span');
            recipeName.className = 'merge-recipe-name';
            recipeName.textContent = recipe.name;

            const quantity = mod.customQuantity || (ingredient.quantity + (ingredient.unit ? ' ' + ingredient.unit : ''));
            const recipeQuantity = document.createElement('span');
            recipeQuantity.className = 'merge-recipe-quantity';
            recipeQuantity.textContent = quantity;

            info.appendChild(recipeName);
            info.appendChild(recipeQuantity);

            label.appendChild(checkbox);
            label.appendChild(info);

            // Create ingredient preview
            const preview = document.createElement('div');
            preview.className = 'merge-ingredient-preview';
            if (mod.merged) {
                preview.classList.add('merged');
            }
            ...
            const ingQuantity = document.createElement('span');
            ingQuantity.className = 'ingredient-quantity';
            ingQuantity.textContent = quantity;

            preview.appendChild(ingName);
            preview.appendChild(ingQuantity);
     ...
     */
    setupMergeModalListeners() {
        const modal = document.getElementById('ingredient-merge-modal');
        if (!modal) return;

        const closeBtn = document.getElementById('close-merge-modal');
        const closeFooterBtn = document.getElementById('merge-modal-close-btn');
        const overlay = modal.querySelector('.modal-overlay');

        const closeModal = () => this.closeMergeModal();
        ...
        const checkboxes = modal.querySelectorAll('.merge-recipe-checkbox');
        checkboxes.forEach(checkbox => {
            checkbox.onchange = (e) => {
                const recipeRefId = parseInt(checkbox.dataset.recipeRefId);
                const ingredientId = checkbox.dataset.ingredientId;
                this.toggleIngredientMerge(recipeRefId, ingredientId, checkbox.checked);
            };
        });
    }

    /**
     * Toggle ingredient merge status
     */
    toggleIngredientMerge(recipeRefId, ingredientId, checked) {
        const { listId, recipes } = this.currentMergeContext;

        console.log('[Merge] Toggle:', { recipeRefId, ingredientId, checked });

        // Find the specific recipe in context that matches this recipeRefId and ingredientId
        const targetRecipe = recipes.find(r => r.recipeRef.id === recipeRefId && r.ingredient.id === ingredientId);
        if (!targetRecipe) {
            console.error('[Merge] Recipe not found in context:', recipeRefId, ingredientId);
            console.log('[Merge] Available recipes:', recipes.map(r => ({ recipeRefId: r.recipeRef.id, ingredientId: r.ingredient.id })));
            return;
        }
        console.log('[Merge] Target recipe found:', targetRecipe.recipe.name);

        // Validate: at least one recipe must remain unchecked
        if (checked) {
            console.log('[Merge] Validating if all would be merged...');
            const allWouldBeMerged = recipes.every(r => {
                const isThisOne = (r.recipeRef.id === recipeRefId && r.ingredient.id === ingredientId);
                const isMerged = r.mod.merged === true;
                const wouldBeMerged = isThisOne || isMerged;
                console.log('[Merge] Recipe:', r.recipe.name, 'recipeRefId:', r.recipeRef.id, 'ingredientId:', r.ingredient.id, 'isThisOne:', isThisOne, 'isMerged:', isMerged, 'wouldBeMerged:', wouldBeMerged);
                return wouldBeMerged;
            });
            console.log('[Merge] All would be merged:', allWouldBeMerged);

            if (allWouldBeMerged) {
                this.showToast('Al menos una receta debe permanecer sin fusionar', 'warning');
                // Uncheck the checkbox
                const checkbox = document.querySelector(`[data-recipe-ref-id="${recipeRefId}"][data-ingredient-id="${ingredientId}"]`);
                if (checkbox) checkbox.checked = false;
                return;
            }
        }
        console.log('[Merge] Validation passed, proceeding to update...');

        // Update merge status and mark as completed if merged
        this.shoppingListManager.updateIngredientModification(
            listId,
            recipeRefId,
            ingredientId,
            { merged: checked, completed: checked }
        );
        console.log('[Merge] Updated merged status for recipeRef:', recipeRefId, 'ingredient:', ingredientId, 'to:', checked);

        // Update the mod in context
        targetRecipe.mod.merged = checked;
        targetRecipe.mod.completed = checked;

        // Calculate merged quantities with updated state
        const mergedQuantity = this.calculateMergedQuantity(recipes);
        console.log('[Merge] Calculated merged quantity:', mergedQuantity);

        // Update customQuantity for non-merged recipes (they receive the concatenated quantity)
        recipes.forEach(r => {
            if (!r.mod.merged) {
                console.log('[Merge] Updating customQuantity for non-merged recipe:', r.recipeRef.id, 'ingredient:', r.ingredient.id);
                this.shoppingListManager.updateIngredientModification(
                    listId,
                    r.recipeRef.id,
                    r.ingredient.id,
        ...
        this.updateMergeModalVisuals();
    }

    /**
     * Calculate merged quantity string
     * @param {Array} recipes - Array of recipe objects
     * @returns {string} Merged quantity like "200g + 150g"
     */
    calculateMergedQuantity(recipes) {
        const nonMerged = [];
        const merged = [];
        ...
        recipes.forEach(r => {
            ...
            if (isMerged) {
                ...
                nonMerged.push(quantity);
            }
        });

        // Format: non-merged + merged
        const allQuantities = [...nonMerged, ...merged];
        return allQuantities.join(' + ');
    }

    /**
     * Update merge modal visuals
     */
    updateMergeModalVisuals() {
        const modal = document.getElementById('ingredient-merge-modal');
        if (!modal) return;

        const { recipes } = this.currentMergeContext;

        recipes.forEach(r => {
            const itemDiv = modal.querySelector(`[data-recipe-ref-id="${r.recipeRef.id}"]`);
            if (!itemDiv) return;

            const preview = itemDiv.querySelector('.merge-ingredient-preview');
            const checkbox = itemDiv.querySelector('.merge-recipe-checkbox');

            if (preview && checkbox) {
                if (checkbox.checked) {
                    preview.classList.add('merged');
                } else {
                    preview.classList.remove('merged');
                }
            }
        });
    }
    ...
    closeMergeModal() {
        ...
        // Save listId before clearing context
        const listId = this.currentMergeContext ? this.currentMergeContext.listId : null;
        const wasExpanded = listId ? this.isListExpanded(listId) : false;

        // Clear context
        this.currentMergeContext = null;
    ...
    /**
     * Toggle manual item completed
     */
    toggleManualItemCompleted(listId, itemId) {
        const list = this.shoppingListManager.getList(listId);
        if (!list || !list.manualItems) return;
    ...
    /**
     * Delete manual item from shopping list
     */
    deleteManualItemFromShoppingList(listId, itemId) {
        ...
        const index = list.manualItems.findIndex(i => i.id === itemId);
        if (index === -1) return;

        list.manualItems.splice(index, 1);
        list.updatedAt = new Date().toISOString();
        this.shoppingListManager.saveLists();
    ...
    /**
     * Delete item from shopping list (legacy)
     * @param {number} listId - List ID
     * @param {number} index - Index of the item to delete
     */
    deleteShoppingItem(listId, index) {
        const list = this.shoppingListManager.getList(listId);
        if (!list) return;
    ...
    saveShoppingList() {
        ...
        if (listId) {
            ...
            if (list) {
                ...
                // Save in new format (manualItems)
                if (!list.manualItems) list.manualItems = [];

                // Map items to manualItems format
                list.manualItems = items.map((item, index) => ({
                    id: list.manualItems[index]?.id || Date.now() + index + Math.random(),
                    name: item.name,
                    quantity: item.quantity,
                    completed: list.manualItems[index]?.completed || false
                }));
            ...
            // Add items as manual items (new format)
            if (!newList.manualItems) newList.manualItems = [];
            items.forEach(item => {
                newList.manualItems.push({
    ...
    closeSelectShoppingListModal() {
        ...
        // Clean up menu data if it exists
        this.currentMenuForShoppingList = null;
    }
    ...
    /**
     * Show modal to select shopping list for all ingredients of a recipe
     * @param {Recipe} recipe - Recipe object with ingredients
     */
    showSelectShoppingListModalForAllIngredients(recipe) {
        if (!recipe || !recipe.ingredients || recipe.ingredients.length === 0) {
            this.showToast('Esta receta no tiene ingredientes', 'error');
            return;
        }
        ...
        this.currentRecipeForShoppingList = recipe;

        // Update modal title
        const modalTitle = modal.querySelector('h2');
        if (modalTitle) {
            modalTitle.textContent = `Añadir ${recipe.ingredients.length} ingredientes a lista`;
        }
        ...
        if (lists.length === 0) {
            listsContainer.innerHTML = '<p class="modal-description">No tienes listas de compra habilitadas. Crea una nueva lista o habilita una existente para añadir estos ingredientes.</p>';
        } else {
            // Render each list as an option
            lists.forEach(list => {
                const option = document.createElement('div');
                ...
                option.addEventListener('click', () => {
                    this.addAllIngredientsToShoppingList(list.id, recipe);
                });
     ...
     * @param {Recipe} recipe - Recipe object with ingredients
     */
    addAllIngredientsToShoppingList(listId, recipe) {
        if (!recipe || !recipe.ingredients || recipe.ingredients.length === 0) {
            this.showToast('Esta receta no tiene ingredientes', 'error');
            return;
        }
        ...
        // Check if recipe already exists in the list (new format)
        if (list.recipes) {
            const existingRecipe = list.recipes.find(r => r.recipeId === recipe.id);
            if (existingRecipe) {
                this.closeSelectShoppingListModal();
                this.currentRecipeForShoppingList = null;
                this.showToast('Esta receta ya está en la lista', 'info');
                return;
            }
        }

        // Add recipe reference (new system)
        this.shoppingListManager.addRecipeReference(listId, recipe.id, '');

        this.closeSelectShoppingListModal();
        this.currentRecipeForShoppingList = null;
        this.showToast(`Receta "${recipe.name}" añadida a la lista`, 'success');
    ...
    /**
     * Convert a menu to a shopping list
     * Shows modal to select which shopping list to add all menu recipes to
     * @param {number} menuId - Menu ID
     */


    /**
     * Show modal to select shopping list for menu recipes
     * @param {Object} menu - Menu object
     */
    showSelectShoppingListModalForMenu(menu) {
        if (!menu) return;

        const modal = document.getElementById('select-shopping-list-modal');
        if (!modal) return;

        // Count recipes in menu
        let recipeCount = 0;
        menu.items.forEach(item => {
            if (this.menuManager.getRecipeIdFromMeal(item, 'lunch')) recipeCount++;
            if (this.menuManager.getRecipeIdFromMeal(item, 'dinner')) recipeCount++;
        });

        // Update modal title
        const modalTitle = modal.querySelector('h2');
        if (modalTitle) {
            modalTitle.textContent = `Añadir menú "${menu.name}" a lista`;
        }

        // Update modal description
        const modalDescription = document.getElementById('select-list-modal-description');
        if (modalDescription) {
            modalDescription.textContent = `Selecciona la lista donde quieres añadir las ${recipeCount} recetas del menú:`;
        }

        // Hide ingredient display (not needed for menu conversion)
        const ingredientDisplay = document.getElementById('ingredient-to-add-display');
        ...
        if (lists.length === 0) {
            listsContainer.innerHTML = '<p class="modal-description">No tienes listas de compra habilitadas. Crea una nueva lista o habilita una existente para añadir estas recetas.</p>';
        } else {
            // Render each list as an option
            lists.forEach(list => {
                const option = document.createElement('div');
                ...
                countSpan.className = 'shopping-list-option-count';
                const totalCount = this.shoppingListManager.getTotalCount(list.id);
                countSpan.textContent = `${totalCount} elementos`;

                option.appendChild(nameSpan);
                option.appendChild(countSpan);

                option.addEventListener('click', () => {
                    this.addMenuRecipesToShoppingList(list.id, this.pendingMenuForShoppingList);
                    this.closeSelectShoppingListModal();
                });
        ...
        // Setup close button and overlay (only once)
        const closeBtn = document.getElementById('close-select-list-modal');
        const overlay = modal.querySelector('.modal-overlay');
        const createNewBtn = document.getElementById('create-new-list-from-modal');
        ...
        if (createNewBtn && !createNewBtn.dataset.listenerAdded) {
            createNewBtn.onclick = () => {
                this.closeSelectShoppingListModal();
                // Create new list with menu name
                const newList = this.shoppingListManager.createList(`Lista de ${menu.name}`);
                this.addMenuRecipesToShoppingList(newList.id, this.pendingMenuForShoppingList);
                // Switch to shopping lists view and expand the new list
                this.showShoppingListsView();
                this.renderShoppingLists();
                setTimeout(() => {
                    this.expandList(newList.id);
            ...
            createNewBtn.dataset.listenerAdded = 'true';
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Debug function to force re-migration of shopping lists
    window.forceMigrateShoppingLists = () => {
        console.log('🔄 Forcing re-migration of all shopping lists...');
        const lists = window.recipeApp.shoppingListManager.lists;
        lists.forEach(list => {
            delete list._migratedToRecipeGroupsV2;
            delete list._migratedToRecipeGroups;
            window.recipeApp.shoppingListManager.migrateListToRecipeGroups(list);
            list._migratedToRecipeGroupsV2 = true;
        });
        window.recipeApp.shoppingListManager.saveLists();
        window.recipeApp.renderShoppingLists();
        console.log('✅ Re-migration complete! Refresh the page.');
    };

    console.log('💡 To force re-migration of shopping lists, run: forceMigrateShoppingLists()');
});
