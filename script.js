...
/**
 * Error messages for recipe scanning
 */
...
/**
 * GeminiAPIService - Manages communication with Gemini API
 */
...
`;
    ...
    async generateContent(prompt) {
        if (!this.hasAPIKey()) {
            throw new Error(SCAN_ERROR_MESSAGES.NO_API_KEY);
        }
    ...
    validateRecipeData(data) {
        ...
        if (validated.ingredients.length === 0) {
            throw new Error(SCAN_ERROR_MESSAGES.NO_INGREDIENTS);
        }
...
/**
 * RecipeScannerService - Orchestrates the complete scanning process
 */
class RecipeScannerService {
    ...
    async scanRecipeFromImage(imageFile, onProgress = null) {
        ...
        try {
            ...
            const text = await this.extractTextWithOCR(optimizedImage, (progress) => {
                this.updateProgress(onProgress, 'Extrayendo texto (OCR)...', 20 + (progress * 40));
            });
            
            if (!text || text.trim().length < 10) {
                throw new Error(SCAN_ERROR_MESSAGES.NO_TEXT_DETECTED);
            }
    ...
    async validateImage(imageFile) {
        ...
        if (!validTypes.includes(imageFile.type)) {
            throw new Error(SCAN_ERROR_MESSAGES.INVALID_IMAGE_FORMAT);
        }
        
        if (imageFile.size > 10 * 1024 * 1024) {
            throw new Error(SCAN_ERROR_MESSAGES.IMAGE_TOO_LARGE);
        }
        
        const dimensions = await this.getImageDimensions(imageFile);
        if (dimensions.width < 300 || dimensions.height < 300) {
            throw new Error(SCAN_ERROR_MESSAGES.IMAGE_TOO_SMALL);
        }
    }
...
class RecipeApp {
    constructor() {
        ...
        this.scannerService = new RecipeScannerService(this.geminiService);
        this.recipes = [];
    ...
    /**
     * Setup AI Scanner event listeners
     */
    setupScannerEventListeners() {
        // Config AI button (desktop)
        const configAIBtn = document.getElementById('config-ai-btn');
        if (configAIBtn) {
            configAIBtn.addEventListener('click', () => {
                this.showAPIKeyModal();
            });
        }

        // Config AI button (mobile)
        const mobileConfigAI = document.getElementById('mobile-config-ai');
        if (mobileConfigAI) {
            mobileConfigAI.addEventListener('click', () => {
                this.showAPIKeyModal();
        ...
        // Scan Modal - Select Image Button
        ...
        // Scan Modal - Image Input
        const scanImageInput = document.getElementById('scan-image-input');
        if (scanImageInput) {
            scanImageInput.addEventListener('change', (e) => {
                if (e.target.files.length > 0) {
                    this.handleScanImageSelected(e.target.files[0]);
                }
            });
        }

        // Scan Modal - Process Button
        const scanProcessBtn = document.getElementById('scan-process-btn');
        if (scanProcessBtn) {
            scanProcessBtn.addEventListener('click', async () => {
                await this.processScanImage();
            });
        }
    ...
    showAPIKeyModal() {
        ...
        if (modal && input) {
            ...
            if (this.geminiService.hasAPIKey()) {
                input.value = this.geminiService.apiKey;
            } else {
                input.value = '';
            }
    ...
    async testAPIKey() {
        ...
        if (!apiKey) {
            status.textContent = 'Por favor ingresa una API Key';
        ...
        const originalKey = this.geminiService.apiKey;
        this.geminiService.apiKey = apiKey;
        ...
        try {
            const isValid = await this.geminiService.testAPIKey();
            
            if (isValid) {
                status.textContent = '✓ API Key válida';
                status.className = 'api-key-status success';
            } else {
                status.textContent = '✗ API Key inválida';
                status.className = 'api-key-status error';
                this.geminiService.apiKey = originalKey;
            }
        } catch (error) {
            console.error('[API Key Test] Error:', error);
            status.textContent = '✗ Error al probar API Key: ' + error.message;
            status.className = 'api-key-status error';
            this.geminiService.apiKey = originalKey;
        } finally {
    ...
    showScanModal() {
        ...
        if (!this.geminiService.hasAPIKey()) {
            alert(SCAN_ERROR_MESSAGES.NO_API_KEY + '\n\nHaz clic en "🤖 Configurar IA" en el menú superior.');
            return;
        }
    ...
    resetScanModal() {
        ...
        this.selectedScanImage = null;
    }

    /**
     * Handle scan image selected
     */
    handleScanImageSelected(file) {
        this.selectedScanImage = file;
        
        // Show preview
        const preview = document.getElementById('scan-preview');
        const previewImage = document.getElementById('scan-preview-image');
    ...
    async processScanImage() {
        if (!this.selectedScanImage) return;
        ...
        try {
            const recipeData = await this.scannerService.scanRecipeFromImage(
                this.selectedScanImage,
                ({ message, percent }) => {
                    if (progressFill) {
            ...
            // Show success
            this.showScanSuccess(recipeData);
            
            // Fill form with data
            await this.fillFormWithScanData(recipeData);
     ...
     * Show scan success
     */
    showScanSuccess(recipeData) {
        const result = document.getElementById('scan-result');
        const ingredientsCount = document.getElementById('scan-ingredients-count');
        const sequencesCount = document.getElementById('scan-sequences-count');
        ...
        if (ingredientsCount) {
            const count = recipeData.ingredients.length;
            ingredientsCount.textContent = `${count} ingrediente${count !== 1 ? 's' : ''}`;
        }
        
        if (sequencesCount) {
            const count = recipeData.sequences.length;
            sequencesCount.textContent = `${count} paso${count !== 1 ? 's' : ''}`;
        }
    }
     ...
     */
    async fillFormWithScanData(recipeData) {
        // Fill name
        const nameInput = document.getElementById('recipe-name');
        ...
        // Fill category
        const categorySelect = document.getElementById('recipe-category');
        if (categorySelect && recipeData.category) {
            categorySelect.value = recipeData.category;
        }
        
        // Fill total time
        if (recipeData.totalTime) {
            const timeMatch = recipeData.totalTime.match(/(\d+)h|(\d+)min/g);
            if (timeMatch) {
                const hoursMatch = recipeData.totalTime.match(/(\d+)h/);
                const minutesMatch = recipeData.totalTime.match(/(\d+)min/);
                ...
                if (minutesInput && minutesMatch) {
                    minutesInput.value = minutesMatch[1];
                }
            }
        }
        
        // Fill caravan friendly
        const caravanCheckbox = document.getElementById('recipe-caravan-friendly');
        if (caravanCheckbox) {
            caravanCheckbox.checked = recipeData.caravanFriendly;
        }
        
        // Fill kitchen appliances
        this.selectedAppliances = recipeData.kitchenAppliances || [];
        this.updateKitchenAppliancesUI();
        ...
        // Fill sequences
        this.sequences = [];
        for (const seq of recipeData.sequences) {
            // Map ingredient names to IDs
            const ingredientIds = seq.ingredientNames
                .map(name => {
                    const ing = this.ingredients.find(i => 
        ...
        console.log('[Scan] Form filled with data:', recipeData);
    }

    // ===== End AI Scanner Methods =====
